...
public class MainActivity extends AppCompatActivity {
    ...
    static {
        try {
            System.loadLibrary("aiassist");  // 只需要加载一次
    ...
    // 创建问题 JSON 的本地方法
    private native String createQuestionJson(String question, long timestamp);
    
    // 解析回答 JSON 的本地方法
    private native DialogData parseAnswerJson(String json);
    ...
    protected void onStart() {
        ...
        mSend.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                dialogData.setFromType(FromType.me);
                dialogData.setType(0);
                dialogData.setContent(mEdit.getText().toString());
                ...
                questionData.question = dialogData.getContent();
                ...
                client.newCall(request).enqueue(new Callback() {
                    ...
                    public void onResponse(Call call, Response response) throws IOException {
                        if (response.isSuccessful()) {
                            ...
                            if (mCjsonLibExisted) {
                                answerData = decodeJsonStrToObj(responseBody);
                                if (answerData != null) {
                                    Log.d(TAG, "3. 使用cJson把字符串转换对象：成功！");
                                } else {
                                    Log.d(TAG, "3. 使用cJson把字符串转换对象：失败！");
                                    // 如果 cJSON 解析失败，使用 Gson 作为备选
                                    Gson gson = new Gson();
                                    answerData = gson.fromJson(responseBody, AnswerData.class);
                                }
                            } else {
                                Gson gson = new Gson();
                                answerData = gson.fromJson(responseBody, AnswerData.class);
                            ...
                            DialogData dialogData = new DialogData();
                            dialogData.setFromType(FromType.ai);
                            dialogData.setType(1);
                            dialogData.setContent(answerData.answer);
                            mDialogList.add(dialogData);
